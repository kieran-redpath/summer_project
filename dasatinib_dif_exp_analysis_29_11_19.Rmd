...
# Find all the genes, note which ones are significant. tt3 is all genes, assign those that are in SignificantGenes as 1's, everything else as 0

#### AllGenes should be created from tt3, not SigSamples/Top2100Genes. You Need Both!.
...
# Data frame version (doesn't work, as you can't easily convert a data frame back to a named binary vector while keeping the names)
plotGenes <- as.data.frame(plotGenes)
significant <- match(Top2100GenesEntrez$ENTREZID, names(plotGenes))
colnames(plotGenes)[1] <- "sigCol"
plotGenes$sigCol[significant] <- "1"

# Doesn't work- I'm missing a bit to tell it that "thing" is T/F (also don't know what to do next)
thing <- which(Top2100GenesEntrez$ENTRxEZID %in% names(plotGenes))
thing <- as.numeric(thing)
important <- plotGenes[match(thing, names(plotGenes))] <- 1

# Attempt No. 3(000): (doesn't work, just returns a 1x1 object of value 1)
pwfGenes <- plotGenes[match(sigGenes, allGenes)] <- 1
View(pwfGenes)
...
## GOseq analysis- The next important bit that needs to change
pwf <- nullp(pwfGenes, 'hg19', id = "knownGene", plot.fit=TRUE)
## Length-corrected
