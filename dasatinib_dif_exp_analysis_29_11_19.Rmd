...
Top150Genes <- as.character(na.omit(SigSamples$Gene_Symbol[1:150]))
# Add Entrez ID's
hs <- org.Hs.eg.db
Top150GenesEntrez <- select(hs, 
                            keys = Top150Genes,
...
dasat_rPAoverrep <- enrichPathway(gene=Top150GenesEntrez$ENTREZID, organism = "human",
...
```{r}
# More stuff
dasat_tstats = g3_vs_g1$t
names(dasat_tstats) = fData(upp)$EntrezGene.ID[ match(rownames(g3_vs_g1),
 fData(upp)$Gene.symbol) ] %>%
 as.vector()
dasat_tstats = sort(dasat_tstats, decreasing=TRUE)
head(dasat_tstats)
## 387103 23397 7272 54443 55143 9319
## 12.57173 12.31195 12.01225 11.89672 11.87608 11.44096
dasat_rPAgsea = gsePathway(dasat_tstats, nPerm=10000,
 minGSSize=50, pvalueCutoff=0.2,
 pAdjustMethod="BH", verbose=FALSE)

dasat_rPAgsea %>% as.data.frame() %>% View()

gseaplot(dasat_rPAgsea, geneSetID = "R-HSA-69278")
```
