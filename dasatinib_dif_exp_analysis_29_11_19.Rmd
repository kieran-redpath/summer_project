...
```{r GoSeq Analysis for top X genes. ACTUALLY USE THIS ONE CAUSE IT'S PROBABLY FILTERED BETTER}
## Load packages
library(reactome.db)
library(goseq)
library(gplots)
...
# Convert gene names using GeneLabelTool, and find the expression data from expData


# Finally, incorporate this gene expression information into a heatmap, as in STAT435
# xx should be a numeric matrix of the expression data

heatmap.2(xx,trace='none',scale='none',Colv=F,Rowv=F,col=greenred(50),keysize=1,
 labCol=paste("Sample",1:ncol(xx)),labRow=paste("Gene",1:ncol(xx)),key=F)
...
heatmap.2(u[,1:2]%*%diag(d[1:2])%*%t(v[,1:2]),trace='none',scale='none',Colv=F,Rowv=F,key=F,
 col=greenred(50),labCol=paste("Sample",1:ncol(xx)),labRow=paste("Gene",1:ncol(xx)))
