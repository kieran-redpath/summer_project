...
+ Builds heatmaps for each pathway, and saves them as .png's.
...
2. Creates a list of the top 400 genes, and uses them for pathway analysis. Plots the results, showing count and adjusted P-value.
```{r}
# 1.
hs <- org.Hs.eg.db
...
# 3.
goseqPathways %>% kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", font_size=8), fixed_thead=T) %>% save_kable(file = "Heatmaps/Enriched_Pathways.html", self_contained = T)
```
...
```{r}
sigPathCor <- matrix(0, length(symbolsinPaths), length(symbolsinPaths))
rownames(sigPathCor) <- colnames(sigPathCor) <- paste0(goseqPathways$Pathway," (", goseqPathways$numDEInCat, ")")
# Above two lines are good

for(a in 1:length(symbolsinPaths)){
  for(b in 1:length(symbolsinPaths)){
    if(a >  b) sigPathCor[a,b] <- length(intersect(symbolsinPaths[[a]], symbolsinPaths[[b]])) / min(length(symbolsinPaths[[a]]), length(symbolsinPaths[[b]]))
    if(a <= b) sigPathCor[a,b] <- length(intersect(symbolsinPaths[[a]], symbolsinPaths[[b]])) / max(length(symbolsinPaths[[a]]), length(symbolsinPaths[[b]]))
  }
}
```
...
* This loop cycles through and creates a heatmap for each pathway, showing the genes involved from the dataset, and their expression level. These heatmaps are saved as .png's.
```{r, warning = FALSE}
...
for(k in 1:length(ensginPaths)){
  ...
  if(nrow(zz) > 1){
    ...
    heatmap.mik(zz[,ord], trace='none', scale='none', col=bluered(50), ColSideColors=cc[,ord], Colv=FALSE, mar=c(4,12), keysize=1, main = (goseqPathways$Pathway[k]))
    legend(0.4,0.97,
