...
+ Prints heatmaps for the top 10 pathways inline.
...
for(k in 1:length(ensginPaths)){
  ...
  if(nrow(zz) > 1){
    ...
    heatmap.mik(zz[,ord], trace='none', scale='none', col=bluered(50), ColSideColors=cc[,ord], Colv=FALSE, mar=c(8,12), keysize=1, main = (goseqPathways$Pathway[k]))
    legend(0.4,0.97,
...
### *Print the top 10 pathways in the document*
1. Creates the "subchunkify" function, used to print the heatmaps with specific dimensions.
2. Prints a legend for the colours used in the bars above the heatmap.
3. Prints the top 10 pathways within the document for easy viewing.

```{r Spare chunk, include=FALSE, eval=FALSE}
# 1.
subchunkify <- function(g, fig_height=12, fig_width=16) {
...
for(k in 1:2){
  ...
  if(nrow(zz) > 1){
    ...
    zz[zz > 3] <- 3
    zz[zz < -3] <- -3
    subchunkify(heatmap.mik(zz[,ord], trace='none', scale='none', col=bluered(50), ColSideColors=cc[,ord], Colv=FALSE, mar=c(8,12), keysize=1, main = (goseqPathways$Pathway[k])),
                max(round(nrow(zz)/6),4), 16)
  }
}

```

<center><pdf src=“summer_project/Heatmaps/Extracellular_matrix_organization_heatmap.pdf" ></center>
