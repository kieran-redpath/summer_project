---
title: "Dasatinib_Pathway_Analysis"
...
* Fix the ordering on the heatmaps
* Fix up the few other things on the post it note
* Spellcheck
* Delete this list
...
* __Setup:__
+ Loads packages.
+ Loads processes data from the CCLE (which is normalised) and the GDSC.
...
+ Finds the intersect between the best results for IC50 and AUC, determining that the relationship between them is important (even though results do not appear significant).

* __Processing IC50 and AUC Differential Expression Data:__
+ Note on errors in the GDSC dataset.
+ Creates new objects to be fed into pathway analyses.
+ Ranks samples based on average adusted P value between IC50 and AUC.
...
+ Builds heatmaps for each pathway, and saves them as PDF's.
...
4. Filters for the three tissues of interest, and removes the middle 50% of samples (based on AUC), so that we're only looking at the extremes.
5. Further processes the expression data, and combines it with the drug data.
6. Creates a tool for determining tissue type based on cell line identifier, or vice versa. Two versions are created, one for all tissues, the other for the three of interest.
7. Filters expression data for the three tissues of interest.
```{r, results='hide'}
# 1.
...
PUT THIS SOMWHERE ELSE AND EXPLAIN IT BETTER
...
All the data is actually -logged, not natural logged. This means there is a positive relationship between AUC and ln IC50, as what is labelled ln IC50 is actually -log IC50, meaning the relationship is reversed (there is a negative correlation between AUC and ln IC50/IC50. The rest of this document assumes that this is true, because if it's not then nothing makes any sense.
...
2. Identifies that the sign of the log fold change is consistent with the IC50 and AUC relationship. As IC50 and AUC have a opposite association with cell line resistance/susceptibility to dasatinib, only samples with -log fold change signs (IC50 log FC sign*AUC log FC sign) are kept.
...
2. Creates a list of the top 400 genes, and use them for pathway analysis. Plot the results, showing count and adjusted P-value.
```{r}
# 1.
hs <- org.Hs.eg.db
...
1. Defines the significant samples. This is based on positive log fold change sign (IC50 sign*AUC sign), and an absolute log fold change of log2(2) for both IC50 and AUC. Rank samples based on adjusted p value, averaged between IC50 and AUC.
2. Cuts down to the top 500 significant samples, for pathway analysis, and add Entrez ID's to these.
3. Adds Entrez ID's to and define non-significant samples.
4. Gets pathway names and extract human-only pathways. Also define the intersect of the significant genes and those that are in reactome.
5. Counts the number of significant genes (1's) and non-significant genes (0's). Find the genes relevant to your dataset.
...
2. Creates a list of significant genes for each goseq pathway, converts them to symbol ID's and adds the information to "goseqPathways"
...
# 3.
goseqPathways %>% kable() %>% kable_styling(bootstrap_options = c("striped", "hover", "condensed", font_size=8), fixed_thead=T)

# %>% save_kable(file = "Heatmaps/Enriched_Pathways.html", self_contained = T)
```
...
2. Creates a color table for the heatmap bars, which lets us sort by tissue type, AUC, or -log IC50.
3. Creates an object showing which gene is part of which pathway.
4. Standardises names between "expDat" and "GeneLabelTool" for readability and code actually working purposes.
```{r}
# 1.
...
* This loop cycles through and creates a heatmap for each pathway, showing the genes involved from the dataset, and their expression level. These heatmaps are saved as PDF's.

### Change it so the PDF's are saved into a sub-directory!!!
```{r}
ord = order(cc["AUC Group",])
...
for(k in 1:length(ensginPaths)){
  ...
  pdf(paste0("Heatmaps/", pn, "_heatmap.pdf"), height=12, width=20)
  heatmap.mik(zz[,ord], trace='none', scale='none', col=bluered(50), ColSideColors=cc[,ord], Colv=FALSE, mar=c(8,12), keysize=1)
  legend(0.4,0.97,
         c("Breast", "High -log IC50", "High AUC", "Large Intestine", "Low -log IC50", "Low AUC", "Stomach", "", ""),
         fill=c("red","yellow","yellow","blue","blue","blue","green","white","white"), ncol=3)
  dev.off()
}
```

### Fix the ordering! Something's gone wrong somewhere, there's a difference between what the heatmaps says is high/low IC50 vs what actually is. It's saying the wrong cell lines are different between the two.
## Figure out how to remove boxes from legend
