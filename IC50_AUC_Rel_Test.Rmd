...
This can be re-used to generate plots for a bunch of different drugs, to quadruple etc stuff.
...
dim(GDSC2)
Lapatinib <- GDSC2 %>% filter(., DRUG_NAME=="Lapatinib")
expDatsplit <- strsplit(colnames(expDat), "_") %>% 
  lapply(., function(x) x[1]) %>% 
  unlist()
colnames(expDat) <- expDatsplit
Lapatinib$CELL_LINE_NAME <- gsub("-","",Lapatinib$CELL_LINE_NAME, fixed=TRUE)
commonSamples <- intersect(Lapatinib$CELL_LINE_NAME,colnames(expDat))
expDat_match <- match(commonSamples, colnames(expDat))
expDat_sort <- expDat[ , na.omit(expDat_match)]
Lapatinib_match <- match(commonSamples, Lapatinib$CELL_LINE_NAME)
Lapatinib_sort <- Lapatinib[na.omit(Lapatinib_match),]
colnames(Lapatinib_sort)[5] <- "CCLE_Name"
Lapatinib_sort$AUC_Level <- ifelse(Lapatinib_sort$AUC < quantile(Lapatinib_sort$AUC, 0.5, na.rm=TRUE), "Low","High")
# expDat Processing
expDat_all <- t(expDat_sort)
expDat_all <- as.data.frame(expDat_all)
setDT(expDat_all, keep.rownames = "CCLE_Name")[]
# "Lapatinib_sort" can now be combined with "expDat_sort"
Lapatinib_all = full_join(x=Lapatinib_sort, y= expDat_all, by=c("CCLE_Name" = "CCLE_Name"))
```

```{r Make a nice clean version of "Lapatinib_sort" that only has BC,CC,GC info}
View(Lapatinib_sort)
Lapatinib_filter <- filter(Lapatinib_sort, TCGA_DESC == "COREAD" | TCGA_DESC == "BRCA" | TCGA_DESC == "STAD")
View(Lapatinib_filter)
expDat_filter <- match(Lapatinib_filter$CCLE_Name, colnames(expDat_sort))
...
dim(expDat_final)
dim(Lapatinib_filter)
head(expDat_final)[,1:5]
head(Lapatinib_filter)[,1:5]
sum(colnames(expDat_final)==Lapatinib_filter$CCLE_Name)
names(Lapatinib_filter)

# lmFit
group <- ifelse(Lapatinib_filter$LN_IC50 > median(Lapatinib_filter$LN_IC50), "High", "Low")
table(group)
boxplot(Lapatinib_filter$LN_IC50 ~ group)
...
hist(Lapatinib_filter$LN_IC50,50)
# Take a quick look at the median, just out of interest
median(Lapatinib_filter$LN_IC50)
head(tt)

#Find the top expression values. Shows that high expression correlates to high ic50, and V.V (e.g. this gene is protective against Lapatinib)
...
# lmFit
group2 <- ifelse(Lapatinib_filter$AUC > median(Lapatinib_filter$AUC), "High", "Low")
table(group2)
boxplot(Lapatinib_filter$AUC ~ group2)
...
hist(Lapatinib_filter$AUC,50)
# Take a quick look at the median, just out of interest
median(Lapatinib_filter$AUC)
head(tt2)

#Find the top expression values. Shows that high expression correlates to low AUC, and V.V (e.g. this gene is protective against Lapatinib)
...
ggplot(data=Lapatinib_filter, aes(x=TCGA_DESC, y=LN_IC50)) +
geom_boxplot() +
geom_beeswarm()

# Split the plot into tissue type and visualise the relationship between AUC and LN_IC50
ggplot(data=Lapatinib_filter, aes(x=AUC, y=LN_IC50)) +
geom_point() +
facet_wrap(~TCGA_DESC)

# Save that figure
ggsave("Lapatinib_relationship.png", ggplot(data=Lapatinib_filter, aes(x=AUC, y=LN_IC50)) +
geom_point() +
facet_wrap(~TCGA_DESC))
```

Do these plots with some other random drugs to figure out what the relationship between LN_IC50 and AUC is/should be/oh god please help.