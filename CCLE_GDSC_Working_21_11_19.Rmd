...
ggplot(data=CCLE_dasat_BCCCGC, mapping=aes(x=CDH1_Mutations, y=AUC, fill=CDH1_Mutations)) +
  ...
  geom_boxplot()+
  geom_beeswarm()
#IC50 ggplot (with CDH1 mutation status)
...
```{r Add CDH1 Expression Data}
# "Low" will be the bottom 20% of expression levels (in "CDH1_Expression")
# "High" will be the top 80% of expression levels (in "CDH1_Expression")
# These can then be plotted on the boxplot
# First create the appropriate column in "CCLE_dasat_BCCCGC", and label everything as "High" by default
CCLE_dasat_BCCCGC$CDH1_Expression="High"
# Attempt to create a data frame that just has data for CDH1 (Ensembl: ENSG00000039068.14), but across every cell line
CDH1_Exp_Levels <- subset(ccleExpData, rownames(ccleExpData)=="ENSG00000039068.14")
#Reorient "CDH1_Exp_Levels" so it has the same orientation as "CCLE_dasat_BCCCGC"
CDH1_Exp_Levels <- t(CDH1_Exp_Levels)
## *Up to Here!*
#Create a data frame for the next set of boxplots. Need to first match "CCLE_Name" on everything, then
Boxplot_data_2 <- match(CCLE_dasat_BCCCGC$CCLE_Name, ccleExpData$colnames)
CCLE_dasat_BCCCGC$CDH1_Expression[Boxplot_data_2] <- "Low"
