...
```{r Misc, include=FALSE, echo=FALSE}
...
```{r IC50 and AUC for Mutants vs Non-mutants}
...
# Use match to combine the two: Test version below
Boxplot_data <- match(cdh1Lines$CCLE_Name, CCLE_dasat_BCCCGC$CCLE_Name)
#This bit doesn't work, just creates a 1x1 table of value 1.
CCLE_dasat_BCCCGC_2 <- CCLE_dasat_BCCCGC$CDH1_Mutations[Boxplot_data] <-  1


# Test The Standard Way
dasat_test <- (subset(CCLE_dasat_BCCCGC, select= -c(AUC, IC50)))
View(dasat_test)
mut_test <- (subset(cdh1Lines, select= -c(AUC, IC50)))
View(mut_test)
Boxplot_test_data <- match(mut_test$CCLE_Name, dasat_test$CCLE_Name)
test <- dasat_test$CDH1_Mutations[Boxplot_test_data] <-  1
