# Load Shiny
library(shiny)
# Load other packages (also outside the Shiny App)
library(limma)
library(edgeR)
library(data.table)
library(magrittr)
...
library(kableExtra)
library(devtools)
# Load Data ( still outside the Shiny App)
# Load CCLE
...
server <- function(input, output) {
  ...
  expDat_sort <- reactive({
    expDatcolnames <- strsplit(colnames(expDat), "_") %>% 
      lapply(., function(x) x[1]) %>% 
      unlist()
    colnames(expDat) <- expDatcolnames
  ...
  drug_sort <- reactive({
    drug_sort <- drug_sort() %>% filter(., TCGA_DESC %in% tcga_filter()$TCGA_DESC)   
  ...
  tt <- reactive({
    ...
    topExp <- expDat_sort2()[match(tt$rn[1], rownames(expDat_sort2())),]
  ...
  tt2 <- reactive({
    ...
    fit2 = lmFit(expDat_sort2(), design2)
    fit2 = eBayes(fit2)
    tt2 = topTable(fit2, coef="HighVsLow", adjust="BH",n=nrow(expDat_sort2()))
    options(digits=4)
    ...
    topExp2 <- expDat_sort2()[match(tt2$rn[1], rownames(expDat_sort2())),]
  ...
  expDat_sort3 <- reactive({
    rownames(expDat_sort2()) <- strsplit(rownames(expDat_sort2()),".", fixed=T) %>% lapply(., function(x) x[1]) %>% unlist()
  })  
  
  # Overlap Heatmap Setup
  sigPathCor <- reactive({
    sigPathCor <- matrix(0, length(SigsymbolsinPaths()), length(SigsymbolsinPaths()))
    rownames(sigPathCor) <- colnames(sigPathCor) <- paste0(goseqPathways2()$Pathway," (", goseqPathways2()$numDEInCat, ")")
  ...
  output$heatmap <- renderPlot({
    ...
    for(k in 1){
      ...
      pathway_expDat_sort <- subset(expDat_sort3(), rownames(expDat_sort3()) %in% ensginPaths[[k]])
      ...
      if(nrow(zz)>1){
        ...
        pn <- gsub(" ","_",goseqPathways2()$Pathway[k])
        png(paste0("Heatmaps/", pn, "_heatmap.png"), width = 20, height = 20, units = 'in', res = 500)
        heatmap.mik(zz[roword,colord], trace='none', scale='none', col=bluered(50), ColSideColors=cc[,colord], Colv=FALSE, RowSideColors=rowcc[roword], Rowv=FALSE, mar=c(4,12), keysize=1, cexRow=l, main=(goseqPathways2()$Pathway[k]))
        legend(0.4,0.97,
