---
title: "Downstream_Analysis_Example_05_12_19"
...
Eg_design = model.matrix(~uppClin$grade)
head(Eg_design)
```
...
```{r, cache=TRUE}
## Fit linear model
Eg_fit = lmFit(uppExpr[, keep], Eg_design) 
Eg_fit = eBayes(Eg_fit)
## Extract results
g3_vs_g1 = topTable(Eg_fit, coef=3, n=nrow(uppExpr), 
                    adjust.method="holm")
...
```{r}
Eg_sigGenes = rownames(g3_vs_g1)[which(sigFC)]
Eg_sigGenes %>%  head(.,30)
strsplit(Eg_sigGenes, "///") %>%  unlist() %>%  unique() %>% length()
```
...
sigEntrez = fData(upp)$EntrezGene.ID[ match(Eg_sigGenes, 
...
uppExprSig = uppExpr[match(Eg_sigGenes, rownames(uppExpr)), ]
dim(uppExprSig)
