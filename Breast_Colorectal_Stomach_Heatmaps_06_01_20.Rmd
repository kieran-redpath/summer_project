---
title: "Breast_Colorectal_Stomach_Heatmaps_06_01_20"
...
orderedsort = dasatinib_sort[order(dasatinib_sort$AUC),]
orderedsort = orderedsort$CCLE_Name
...
for(k in 1:length(ensginPaths)){
  ...
  heatmap.2(zz[, ord], trace='none', scale='none', col=bluered(50), ColSideColors=cc, Colv = FALSE)
  dev.off()
}
...
ord <- subset(orderedsort, subset=which(colnames(zz) %in% orderedsort), select=which(colnames(zz) %in% orderedsort))

intzz <- intersect(orderedsort, colnames(zz))
zz_match <- match(intzz, colnames(zz))
zz_ord <- orderedsort[na.omit(zz_match) , ]
